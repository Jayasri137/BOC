...
$intake_year  = $this->db->get('intake_year');

// Get all universities with country relationship for client-side filtering
$all_universities = $this->db->select('u.id, u.name, u.country_id, c.name as country_name')
  ->from('university u')
...
?>
<?php $enquiry_types = $this->db->get_where('enquiry_type', ['status' => 'Active']); ?>
<div class="content-wrapper">
  <div class="container-full">
    ...
    <div class="content-header d-flex align-items-center justify-content-between">
      <div>
        <h3 class="page-title">Create Enquiry</h3>
        <nav>
          <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item">Create Enquiry</li>
          </ol>
        </nav>
      </div>
      <div>
    ...
    <div class="container border p-5 mt-4 mb-4 bg-white rounded shadow-sm">
      ...
      <form id="enquiryForm" action="<?= base_url('enquiry/create_enquiry'); ?>" method="post" novalidate>
            ...
            <div class="row g-3">
              ...
              <div class="col-md-3">
                <div class="form-group">
                  ...
                  <div class="input-group">
                    <span class="input-group-text" id="countryCodeLabel">+--</span>
              ...
              <div class="col-md-3">
                <div class="form-group">
                  ...
                  <select name="enquiry_type_id" class="form-control">
                    ...
                    <?php if ($enquiry_types && $enquiry_types->num_rows() > 0): ?>
                      <?php foreach ($enquiry_types->result() as $etype): ?>
              ...
              <div class="col-md-3">
                <div class="form-group">
                  ...
                  <?php
                  $ci = &get_instance();
                  $role = $ci->db->query("SELECT * FROM user_group WHERE ug_id != 1 ORDER BY ug_id DESC");
                  ?>
                  <select name="user_role_id" class="form-control" required>
                    <option value="">Select Role</option>
                    <?php if ($role && $role->num_rows() > 0): ?>
                      <?php foreach ($role->result() as $roles): ?>
                        <option value="<?= $roles->ug_id ?>">
                          <?= htmlspecialchars($roles->ug_name) ?>
                        </option>
                      <?php endforeach; ?>
                    <?php else: ?>
                  ...
                  <select id="intakeSelect" name="intake_year_id" class="form-control" required>
                    ...
                    <?php if ($intake_year && $intake_year->num_rows() > 0): ?>
                      <?php foreach ($intake_year->result() as $iy): ?>
                        <option value="<?= $iy->id ?>"><?= htmlspecialchars($iy->intake_year ?? '-') ?></option>
                      <?php endforeach; ?>
                    <?php endif; ?>
                  </select>
                </div>
              ...
              <div class="col-md-3">
                <div class="form-group">
                  ...
                  <select id="universitySelect" name="university_id" class="form-control">
                    ...
                    <!-- Universities will be loaded dynamically based on country selection -->
                  </select>
                </div>
              </div>
                ...
                <tbody>
                  ...
                  <?php foreach ($quals as $key => $label): ?>
                    <tr>
                      ...
                      <td><input type="text" name="academic[<?= $key ?>][school]" class="form-control" placeholder="School/College name"></td>
                      <td><input type="text" name="academic[<?= $key ?>][marks]" class="form-control" placeholder="Marks/GPA"></td>
                      <td><input type="text" name="academic[<?= $key ?>][english_marks]" class="form-control" placeholder="English marks %"></td>
                      <td><input type="text" name="academic[<?= $key ?>][passed_out]" class="form-control" placeholder="Year"></td>
                    </tr>
                  <?php endforeach; ?>
                </tbody>
              </table>
        ...
        <div class="box-footer d-flex justify-content-between mt-4">
          ...
          <button type="submit" class="btn btn-success">
            <i class="ti-save-alt"></i> Submit Enquiry
          </button>
        </div>
