...
<?php $enquiry = $this->db->query("select * from enquiry")->row(); ?>
<div class="col-md-4">
<div class="form-group">
    <label>Username</label>
    <input type="text" name="username" class="form-control" value="<?= htmlspecialchars($lead->username ?? ''); ?>" placeholder="Enter username">
</div>
</div>

<?php 
// Fetch enquiries from database
$enquiries = $this->db->get('enquiry')->result();
?>
...
// Fetch all enquiry types directly in view
$ci->db->select('*');
$ci->db->from('enquiry_type');
$ci->db->where('status', 'Active');
$ci->db->order_by('enquiry_type', 'ASC');
$enquiry_types = $ci->db->get()->result();

// Fetch lead enquiry_type if it exists (for edit view)
$lead_enquiry_type = '';
if (isset($lead) && !empty($lead->enquiry_type_id)) {
    $ci->db->select('enquiry_type');
    $ci->db->from('enquiry_type');
    $ci->db->where('id', $lead->enquiry_type_id);
    $enquiry_type_row = $ci->db->get()->row();
    if ($enquiry_type_row) {
        $lead_enquiry_type = $enquiry_type_row->enquiry_type;
    }
}
?>
...
<div class="col-md-4">
    <div class="form-group">
        ...
        <select name="enquiry_type_id" class="form-control">
            ...
            <?php if (!empty($enquiry_types)): ?>
                <?php foreach ($enquiry_types as $enquiry_type): ?>
                    <option value="<?php echo htmlspecialchars($enquiry_type->id); ?>" 
                        <?php echo (isset($lead->enquiry_type_id) && $lead->enquiry_type_id == $enquiry_type->id) ? 'selected' : ''; ?>>
                        <?php echo htmlspecialchars($enquiry_type->enquiry_type); ?>
                    </option>
                <?php endforeach; ?>
            <?php endif; ?>
        </select>
        
        <!-- Display current enquiry type if it exists (for reference) -->
