...
<style>
/* Same UI style — minimal and clean */
...
// 4. Fetch record safely
$enquiry = null;
if (!empty($id)) {
    $enquiry = $this->db->get_where('web_enquiry', ['id' => $id])->row();
}
?>
  ...
  <div class="container-full">
    <div class="content-header mb-3">
      <div class="d-flex align-items-center justify-content-between">
        <div class="me-auto p-3">
          <h3 class="page-title mb-1">Enquiry Details</h3>
          <div class="small text-muted">View enquiry record by ID</div>
        </div>
        <div>
          <a href="<?= base_url('webenquiry'); ?>" class="btn btn-primary btn-sm">
    ...
    <div class="container">
      <?php if (!empty($enquiry)): ?>
        <?php
          $id          = (int)$enquiry->id;
          $full_name   = trim(($enquiry->first_name ?? '') . ' ' . ($enquiry->last_name ?? ''));
          $email       = $enquiry->email ?? '-';
          $mobile      = $enquiry->mobile ?? '-';
          $study       = $enquiry->study_level ?? '-';
          $dest        = $enquiry->your_studies ?? '-';
          $office      = $enquiry->near_office ?? '-';
          $designation = $enquiry->designation ?? '-';
          $created     = !empty($enquiry->created_at) ? date('d-M-Y', strtotime($enquiry->created_at)) : '-';
        ?>

        <div class="enq-card">
          <div class="enq-header">
            <div>
              <h5 style="margin:0; font-size:1rem;"><i class="fa fa-envelope"></i> <?= htmlspecialchars($full_name ?: '—'); ?></h5>
          ...
          <div class="enq-body">
            <div class="info-grid">
              ...
              <div class="info-block">
                ...
                <span class="info-value"><?= htmlspecialchars($study ?: '-'); ?></span>
              </div>

              <div class="info-block">
                <span class="info-label">Start Year / Your Studies</span>
                <span class="info-value"><?= htmlspecialchars($dest ?: '-'); ?></span>
              </div>

              <div class="info-block">
                <span class="info-label">Nearest Office</span>
                <span class="info-value"><?= htmlspecialchars($office ?: '-'); ?></span>
              </div>
      ...
      <?php else: ?>
        <div class="alert alert-warning text-center mt-3 alert-small">
          ...
          No enquiry found for the provided ID.
          <?php if (empty($id)): ?>
